      ...
      if (request.body?.event === "messages.upsert") {
        ...
        if (request.body?.data?.key?.fromMe === true && request.body?.data?.key?.remoteJid === '5515991957645@s.whatsapp.net') {
          ...
          if (messageType === "imageMessage") {
            ...
            console.log("request.body?.data", request.body?.data);
            console.log("request.body?.data?.message.imageMessage.jpegThumbnail?.contextInfo", request.body?.data?.message?.imageMessage?.jpegThumbnail?.contextInfo);
            ...
            if (assistantMessage) {
              ...
              const pixValueImage = (this.pixValue && typeof this.pixValue === 'number') ? this.pixValue : Number(this.pixValue?.toString().replace(',', '.'));
